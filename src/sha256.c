/* 
 * May 2015
 * Edited by Yidong Ren for 8051 MCU and SDCC
 * 
 * Original license:
 * 
...
static void polarssl_zeroize( void *v, size_t n ) {
    __data volatile unsigned char *p = v; while( n-- ) *p++ = 0;
}
...
// For SDCC only! since SDCC store vars in little endian
#define GET_UINT32_BE(n,b,i)                            \
{                                                       \
    ((unsigned char *)&(n))[0] = (b)[(i) + 3];          \
    ((unsigned char *)&(n))[1] = (b)[(i) + 2];          \
    ((unsigned char *)&(n))[2] = (b)[(i) + 1];          \
    ((unsigned char *)&(n))[3] = (b)[(i) + 0];          \
}
#endif
...
// Rewite as fucntion to reduce code size
uint32_t S0(__data uint32_t x) {
    return ROTR(x, 7) ^ ROTR(x,18) ^  SHR(x, 3);
}
uint32_t S1(__data uint32_t x) {
    return ROTR(x,17) ^ ROTR(x,19) ^  SHR(x,10);
}
uint32_t S2(__data uint32_t x) {
    return ROTR(x, 2) ^ ROTR(x,13) ^ ROTR(x,22);
}
uint32_t S3(__data uint32_t x) {
...
{
    __data uint32_t temp1, temp2;
    __xdata uint32_t W[64];
    __idata uint32_t A, B, C, D, E, F, G, H;
...
__code static const unsigned char sha256_padding[64] =
{
 ...
 */
#define sha256(input,ilen,output,is224 )                        \
{                                                               \
...
*/ //commented out to reduce ram use, thus keylen must <= 64 !!! 
    ...
    //polarssl_zeroize( sum, sizeof( sum ) );             //commented out to reduce ram use, thus keylen must <= 64 !!! 
}
...
void sha256_hmac_finish( __xdata sha256_context *ctx, unsigned char output[32] )
{
    __bit is224;
    char hlen;
    __xdata unsigned char tmpbuf[32];
 ...
 */
#define sha256_hmac(key,keylen,input,ilen,output,is224 )        \
{                                                               \
    __xdata sha256_context ctx;                                 \
    sha256_init( &ctx );                                        \
